<template>
    <div 
        ...
        <div v-for="(product, index) in getProductsInCart" :key="index" class="box-item">
            ...
            <h3
                class="item-name">{{ product.name }}</h3>
            <span
                class="item-amount">Quantity: {{product.quantity}}</span>
            <span
                class="item-price">{{product.quantity}} * {{ product.price }}$</span>
        </div>
        <div
            class="cart-info" v-if="hasProduct()">
